...
import type { ForgeArtifactData } from '../../types/artifact';
import { createScopedLogger } from '../../utils/logger';
import { unreachable } from '../../utils/unreachable';

const ARTIFACT_TAG_OPEN = '<forgeArtifact';
const ARTIFACT_TAG_CLOSE = '</forgeArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<forgeAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</forgeAction>';
const BOLT_QUICK_ACTIONS_OPEN = '<forge-quick-actions>';
const BOLT_QUICK_ACTIONS_CLOSE = '</forge-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends ForgeArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: ForgeArtifactData;
  currentAction: ForgeActionData;
  actionId: number;
}
    ...
    while (i < input.length) {
      if (input.startsWith(BOLT_QUICK_ACTIONS_OPEN, i)) {
        ...
        if (actionsBlockEnd !== -1) {
          ...
          // Find all <forge-quick-action ...>label</forge-quick-action> inside
          const quickActionRegex = /<forge-quick-action([^>]*)>([\s\S]*?)<\/forge-quick-action>/g;
          let match;
          const buttons = [];
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as ForgeAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as ForgeAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__forgeQuickAction__"',
    'data-forge-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__forgeQuickAction__\" data-forge-quick-action=\"true\">${buttons.join('')}</div>`;
}
