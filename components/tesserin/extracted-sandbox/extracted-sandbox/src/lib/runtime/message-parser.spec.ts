...
describe('StreamingMessageParser', () => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ['Foo bar <for', 'Foo bar '],
      ['Foo bar <forge', 'Foo bar '],
      ['Foo bar <forgea', 'Foo bar <forgea'],
      ['Foo bar <forgeA', 'Foo bar '],
      ['Foo bar <forgeArtifacs></forgeArtifact>', 'Foo bar <forgeArtifacs></forgeArtifact>'],
      ['Before <orgeArtfiact>foo</forgeArtifact> After', 'Before <orgeArtfiact>foo</forgeArtifact> After'],
      ['Before <forgeArtifactt>foo</forgeArtifact> After', 'Before <forgeArtifactt>foo</forgeArtifact> After'],
    ])('should correctly parse chunks and strip out forge artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <forgeArtifact title="Some title" id="artifact_1">foo bar</forgeArtifact> Some more text',
        {
      ...
      [
        [
          'Some text before <forgeArti',
          'fact',
          ' title="Some title" id="artifact_1" type="bundled" >foo</forgeArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' ',
          '>',
          'foo</forgeArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' title="Some title" id="artifact_1"',
          ' >fo',
          'o</forgeArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <forgeArtifact title="Some title" id="artifact_1">foo</forgeArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <forgeArtifact title="Some title" id="artifact_1"><forgeAction type="shell">npm install</forgeAction></forgeArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <forgeArtifact title="Some title" id="artifact_1"><forgeAction type="shell">npm install</forgeAction><forgeAction type="file" filePath="index.js">some content</forgeAction></forgeArtifact> After',
        {
          output: 'Before  After',
