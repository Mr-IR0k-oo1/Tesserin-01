...
export const filesToArtifacts = (files: Array<{ path: string; content: string } | string>, id: string): string => {
  const finalMessage = `
User updated the following files:

<forgeArtifact id="${id}" title="User Updated Files">
  ${files
    .map((file) => {
      const filePath = typeof file === 'string' ? file : file.path;
      return `<forgeAction type="file" filePath="${filePath}">
${typeof file === 'string' ? '' : file.content}
</forgeAction>`;
    })
    .join('\n')}
</forgeArtifact>
