  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__forgeThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__forgeArtifact__', '__forgeThought__', '__forgeQuickAction', '__forgeSelectedElement__'],
    ...
    button: [
      ...
      'name',
      'value',
      ['className', '__forgeArtifact__', '__forgeThought__', '__forgeQuickAction'],
    ],
  },
  strip: [],
};
