...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<forgeAction type="shell">${commands.setupCommand}</forgeAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<forgeAction type="start">${commands.startCommand}</forgeAction>
`;
  }
...
<forgeArtifact id="project-setup" title="Project Setup">
${commandString}
</forgeArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeForgeArtifactTags(input: string) {
  // Regular expression to match forgeArtifact tags and their content
  const regex = /(<forgeArtifact[^>]*>)([\s\S]*?)(<\/forgeArtifact>)/g;
...
export function escapeForgeActionTags(input: string) {
  // Regular expression to match forgeArtifact tags and their content
  const regex = /(<forgeAction[^>]*>)([\s\S]*?)(<\/forgeAction>)/g;
...
export function escapeForgeTags(input: string) {
  return escapeForgeArtifactTags(escapeForgeActionTags(input));
}
